...
def cubic_spline(inputs,
    ...
    if inverse:
        bin_idx = math.searchsorted(cumheights, inputs)[..., None]
    else:
        bin_idx = math.searchsorted(cumwidths, inputs)[..., None]
    ...
    if inverse:
        ...
        p = math.cbrt((-depressed_1[one_root_mask] + torch.sqrt(-discriminant[one_root_mask])) / 2.)
        q = math.cbrt((-depressed_1[one_root_mask] - torch.sqrt(-discriminant[one_root_mask])) / 2.)
