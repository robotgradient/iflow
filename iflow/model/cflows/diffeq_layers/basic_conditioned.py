...
## Concatenating Conditioning information and input
class CondIgnoreLinear(nn.Module):
    def __init__(self, dim_in, dim_out, dim_cond):
        super(CondIgnoreLinear, self).__init__()
        self._layer = nn.Linear(dim_in + dim_cond, dim_out)

    def forward(self, t, x, c):
        x_concat = torch.cat((x,c),axis=1)
        return self._layer(x_concat)
