...
class RegularizedCODEfunc(nn.Module):
    def __init__(self, odefunc, regularization_fns):
        super(RegularizedCODEfunc, self).__init__()
    ...
    def forward(self, t, state):
        ...
        with torch.enable_grad():
            ...
            dstate = self.odefunc(t, (x, logp, c))
            if len(state) > 3:
                dx, dlogp = dstate[:2]
...
def quadratic_cost(x, logp, dx, dlogp, context):
    del x, logp, dlogp
    dx = dx.view(dx.shape[0], -1)
    return 0.5*dx.pow(2).mean()

def jacobian_frobenius_regularization_fn(x, logp, dx, dlogp, context):
    del logp, dlogp, dx, x
    return context.sqjacnorm.mean()
